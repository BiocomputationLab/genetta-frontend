...
nv_der = str(model.identifiers.external.synonym)
nv_conf = str(model.identifiers.external.confidence)
...
bl_ints = [nv_activated, nv_modifier, nv_product, nv_repressed, nv_template]
...
class TruthNameSynonym(AbstractExpansion):
    ...
    def expand(self):
        ...
        for entity in self._tg.get_physicalentity():
            try:
                e_synonyms = [s.v.get_key() for 
                              s in s_graph.synonyms(entity)]
            except ValueError:
                e_synonyms = []
        ...
        for name, entities in name_map.items():
            ...
            name = ReservedNode(name, o_synonym, graph_name=self._tg.name)
            edges = [
                ReservedEdge(
                    entity, name, nv_der, 
                    **{nv_conf: conf, "graph_name": self._tg.name}
                )
                for entity in entities
            ]
            self._tg.add_edges(edges)
